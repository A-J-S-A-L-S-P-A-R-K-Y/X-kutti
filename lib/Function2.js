...
exports.GIFBufferToVideoBuffer = async (image) => {
  ...
  await fs.writeFileSync(`./lib/Cache/${filename}.gif`, image);
  child_process.exec(
    `ffmpeg -i ./lib/Cache/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./lib/Cache/${filename}.mp4`
  );
  await sleep(4000);

  var buffer5 = await fs.readFileSync(`./lib/Cache/${filename}.mp4`);
  Promise.all([
    unlink(`./lib/Cache/${filename}.mp4`),
    unlink(`./lib/Cache/${filename}.gif`),
  ]);
  return buffer5;
};
